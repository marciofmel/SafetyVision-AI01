...
export default function SetoresTecnico() {
  ...
  const handleSave = async (id?: string) => {
    if (!form.nome.trim()) return toast.error('Nome do setor é obrigatório');
    if (!form.empresaId) return toast.error('Selecione a empresa');
    try {
      if (id) { await api.put(`/setores/${id}`, { nome: form.nome, descricao: form.descricao }); toast.success('Setor atualizado!'); }
  ...
  return (
    <div>
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-extrabold text-navy-900">Seções / Setores</h1>
