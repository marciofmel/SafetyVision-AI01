...
router.get('/:inspecaoId/relatorio', async (req: AuthRequest, res) => {
  try {
    ...
    ry = mtop + mrowH + gravidades.length * mrowH + 10;
    ...
    doc.rect(50, ry, totalW, 22).fillAndStroke('#0F172A', '#0F172A');
    doc.fontSize(9).font('Helvetica-Bold').fillColor('#FFFFFF');
    prazoHeads.forEach((h, i) => doc.text(h, cLefts[i] + 4, ry + 7));
    doc.font('Helvetica');
    ry += 22;
    ...
    gravidades.forEach((g) => {
      ...
      if (count === 0) return;
      const cor = gravCor[g];
      doc.rect(50, ry, totalW, 20).fillAndStroke('#F8FAFC', '#E2E8F0');
      doc.fontSize(9).font('Helvetica-Bold').fillColor(cor).text(gravLabel[g], cLefts[0] + 4, ry + 5);
      doc.font('Helvetica').fillColor('#334155');
      doc.text(`${count}`, cLefts[1] + 4, ry + 5);
      doc.text(prazos[g], cLefts[2] + 4, ry + 5);
      doc.text(g === 'critica' ? 'IMEDIATA' : g === 'alta' ? 'ALTA' : g === 'media' ? 'MÉDIA' : 'BAIXA', cLefts[3] + 4, ry + 5);
      doc.text(`${inspecao.usuario.nome}`, cLefts[4] + 4, ry + 5, { width: colW[4] - 8 });
      ry += 20;
    });

    if (inspecao.riscos.length === 0) {
      doc.fontSize(10).font('Helvetica').fillColor('#16A34A').text('Nenhum risco foi identificado nesta inspeção. Ambiente dentro dos padrões esperados.', 50, ry + 6, { width: 495 });
    }
    ...
    if (inspecao.observacoes) {
      ...
      ry += 16;
      doc.fontSize(10).font('Helvetica').fillColor('#374151').text(inspecao.observacoes, 50, ry, { width: 495, height: 100, lineGap: 3 });
    }
    ...
    if (inspecao.riscos.length > 0) {
      ...
      for (let i = 0; i < inspecao.riscos.length; i++) {
        ...
        doc.roundedRect(50, y, 495, 90, 8).fillAndStroke('#F8FAFC', '#E2E8F0');
        doc.circle(70, y + 12, 10).fill(cor);
        doc.fontSize(10).font('Helvetica-Bold').fillColor('#FFFFFF').text(`${i + 1}`, 64, y + 8, { width: 12, align: 'center' });

        doc.fontSize(11).font('Helvetica-Bold').fillColor('#0F172A').text(risco.descricao, 90, y + 6, { width: 430 });
        doc.fontSize(9).font('Helvetica').fillColor('#64748B');
        doc.text(`Categoria: ${risco.categoria} | Gravidade: ${risco.gravidade.toUpperCase()} | Confiança: ${(risco.confianca * 100).toFixed(0)}%`, 90, y + 22, { width: 430 });
        doc.text(`NRs: ${risco.nrsRelacionadas || '---'} | Local: ${risco.localIdentificado}`, 90, y + 34, { width: 430 });
        doc.text(`Consequências: ${risco.consequencias}`, 90, y + 46, { width: 430 });

        doc.fontSize(8).font('Helvetica').fillColor('#059669');
        doc.text(`Prevenção: ${risco.medidasPreventivas}`, 90, y + 58, { width: 430 });
        doc.text(`Correção: ${risco.medidasCorretivas}`, 90, y + 68, { width: 430 });
    ...
    if (inspecao.epiViolacoes.length > 0) {
      ...
      for (const epi of inspecao.epiViolacoes) {
        ...
        doc.roundedRect(50, y, 495, 30, 6).fillAndStroke(epi.status === 'ausente' ? '#FEF2F2' : epi.status === 'incorreto' ? '#FFF7ED' : '#F0FDF4', '#E2E8F0');
        doc.circle(70, y + 15, 8).fill(cor);
        doc.fontSize(8).font('Helvetica-Bold').fillColor('#FFFFFF').text(epi.status === 'ausente' ? 'X' : epi.status === 'incorreto' ? '!' : '✓', 64, y + 12, { width: 12, align: 'center' });

        doc.fontSize(11).font('Helvetica-Bold').fillColor('#0F172A').text(`${epi.epiNome}`, 90, y + 8, { width: 200 });
        doc.fontSize(9).font('Helvetica').fillColor(cor).text(statusLabel, 300, y + 8, { width: 100 });
        doc.fontSize(8).fillColor('#64748B').text(`Confiança: ${(epi.confianca * 100).toFixed(0)}%`, 420, y + 10, { width: 110 });

        if (epi.descricao) {
          doc.fontSize(8).fillColor('#64748B').text(epi.descricao, 90, y + 20, { width: 440 });
        }
    ...
    if (fotos.length > 0) {
      ...
      for (const foto of fotos) {
        ...
        try {
          ...
          doc.fontSize(8).font('Helvetica').fillColor('#64748B').text(foto.nome, x, y + finalH + 2, { width: finalW });

          col = col === 0 ? 1 : 0;
          if (col === 0) y += finalH + 18;
        } catch {
          // skip broken images
        }
      }
    ...
    doc.text(`Técnico Responsável: ${inspecao.usuario.nome}`, 50, 85);
