...
import { FiPlus, FiEdit2, FiTrash2, FiSave, FiX, FiSearch, FiLoader, FiCheckCircle } from 'react-icons/fi';
...
export default function EmpresasTecnico() {
  ...
  const handleCnpjLookup = async (value: string) => {
    ...
    try {
      ...
      const enderecoParts = [data.endereco, data.bairro, data.cidade && data.estado ? `${data.cidade}/${data.estado}` : ''].filter(Boolean);
      setForm({
  ...
  const handleSave = async (id?: string) => {
    try {
      ...
      setShowNew(false); setEditingId(null); setForm({ nome: '', cnpj: '', endereco: '', telefone: '', email: '' }); setCnpj(''); setCnpjFound(false); load();
  ...
  return (
    <div>
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button onClick={() => { setShowNew(true); setForm({ nome: '', cnpj: '', endereco: '', telefone: '', email: '' }); setCnpjFound(false); }} className="btn-primary">
      ...
      {showNew && (
        <div className="card mb-6 p-4 sm:p-6">
          ...
          {cnpjFound && (
            <p className="mb-3 text-xs text-navy-500">Dados preenchidos automaticamente pela busca. Você pode editar livremente.</p>
          )}
          ...
          <div className="mt-4 flex gap-2">
            ...
            <button onClick={() => { setShowNew(false); setEditingId(null); setCnpjFound(false); }} className="btn-primary bg-navy-200 text-navy-700 hover:bg-navy-300"><FiX size={16} /> Cancelar</button>
          </div>
        </div>
      )}
          ...
          <div key={e.id} className="card p-4 transition-all hover:shadow-md">
            <div className="flex items-start justify-between">
              ...
              <div className="flex gap-1">
                <button onClick={() => { setEditingId(e.id); setForm({ nome: e.nome, cnpj: e.cnpj || '', endereco: e.endereco || '', telefone: e.telefone || '', email: e.email || '' }); setShowNew(true); setCnpjFound(false); }} className="rounded-lg p-2 text-navy-400 hover:bg-navy-100"><FiEdit2 size={14} /></button>
